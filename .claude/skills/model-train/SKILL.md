...
4. record：聚合配置/metric/best.pth/简报历史，生成 `<OUTPUT_ROOT>/<YYYYMMDDHH>_<模型>_<备注>.md`（与训练产物同目录），并建 `<OUTPUT_ROOT>/asset/` 占位图像目录
...
脚本只产**原料**（`<OUTPUT_ROOT>/<时间戳>_<模型>_<tag>.md` + best.pth + `<OUTPUT_ROOT>/eval` + `<OUTPUT_ROOT>/asset/`），**报告文本由 LLM 手动撰写**，对齐 [`实验报告模板.md`](实验报告模板.md)（同目录下）的六章结构（实验设置 / 核心结果 / 关键排查 / 评价 / 后续 debug / 产物清单）。模板已留好 `<占位符>`，LLM copy 骨架填充即可。

**触发时机**：`autofinish` 跑完 `val → pickbest → record` 后，**LLM 一次性手写报告**；不靠 cron，不靠会话内 poll 触发。

**报告 + 图像落点（统一规则，单一真相源）**：

所有报告与图像**都落在 OUTPUT_ROOT 内**（`output/train_log/<数据集>/<名字>/`）——与训练产物（ckpt / eval / brief.log）同目录，不再散落到 `experiments/` 或 `note/`：

| 类型 | 落点 | 说明 |
|---|---|---|
| **实验记录（record 产物）** | `<OUTPUT_ROOT>/<YYYYMMDDHH>_<模型>_<tag>.md` | `record` 子命令自动生成（原料，含配置/metric/best/简报历史） |
| **训练报告（LLM 撰写）** | `<OUTPUT_ROOT>/<YYYYMMDDHH>_<模型>_<tag>_报告.md` | LLM 在 record 产物基础上手写，六章结构 |
| **图像资产** | `<OUTPUT_ROOT>/asset/` | 所有图统一落此处 |

- `OUTPUT_ROOT = output/train_log/<数据集>/<YYYYMMDDHH>_<模型>_<备注>/`
- 例：`output/train_log/vod/2026072222_radarnext_mdfen_0722_paper/2026072311_radarnext_mdfen_0722_paper.md` + 同目录 `asset/`

**训练产物：训完自动落袋（resbag skill）**：

- model-train autofinish 在 record 步骤后自动调 `resbag make`（详见 [.claude/skills/resbag/SKILL.md](../resbag/SKILL.md) + [resbag 设计 spec](../../../docs/superpowers/specs/2026-07-24-resbag-skill-design.md)）。
- 落袋目录：`<OUTPUT_ROOT>/resbag/`（硬复制 cfg/sh/log/eval/best/last/asset/8 类文件）+ `<OUTPUT_ROOT>/model_store.yaml`（单实验总览）。
- 跨实验聚合：`resbag list --dataset vod`（view 层聚合，不写盘）。
- 报告落点：`resbag/README.md`（取代之前 `note/<模型>/` 散落），图像引用 `resbag/asset/`。

**图像引用约定**（报告与 asset 同在 `resbag/` 内，相对路径直接 `asset/`）：

- **图一律放 `<OUTPUT_ROOT>/resbag/asset/`**——硬复制自 `<OUTPUT_ROOT>/asset/`，含 `loss_curve.png`、`tb_loss_curves.png`、`<模型>_frames/frame_NNNNN.png` 等
- 报告内用 markdown 相对链接：`![alt](asset/xxx.png)`（**不带 `../`**，因为报告与 asset/ 同在 resbag/）

**报告输入原料**（LLM 从以下源头取数据，不用 grep 瞎找）：

- **结构化权威**：`<OUTPUT_ROOT>/model_store.yaml`（resbag 产物，含 mAP_R40 三类+均值、params_m/flops_g/commit/seed/optimizer/ts/status）
- mAP 原始：`resbag/eval_results.json`（直接读 `ret_dict`，不解析 log）
- cfg：`resbag/cfg.yaml`（git HEAD 时快照）
- best/last 权重：`resbag/best.pth` / `resbag/last.pth`
- 简报历史：`<OUTPUT_ROOT>/brief.log`
- 训练 log：`resbag/train.log`

> 报告含主观判断（gap 归因、后续 debug 优先级），**LLM 写，脚本不替写**——脚本替写会把判断偷渡成代码。
> 数据源已从 record md（LLM 撰写易被人改）切换到 resbag 结构化产物（cfg+results.json 权威）。
...
| 「图片我复制一份到 note」 | 图一律落 `<OUTPUT_ROOT>/asset/`，报告内用相对链接 `asset/xxx.png` 引用，**不拷贝到 note**。报告与图同在 OUTPUT_ROOT 内，删训练目录即连带清理，无孤儿资产。 |

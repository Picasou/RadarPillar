#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— a2（A2=SEBlock）
...
CFG="experiments/YAML/a2.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="a2"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_a2"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:a2] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

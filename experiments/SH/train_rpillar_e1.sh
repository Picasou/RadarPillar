#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— e1（E1 关速度分解）
...
CFG="experiments/YAML/e1.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="e1"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_e1"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:e1] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

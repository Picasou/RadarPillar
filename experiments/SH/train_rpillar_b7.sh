#!/bin/bash
# RPiN 阶段2 补强完整训练 .sh（80 epochs）— b7（RepDWCNoneBackbone，OUT_CHANNELS=[64, 128, 256]，对齐 standard b3）
...
CFG="experiments/YAML/b7.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="b7"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_b7"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:b7] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

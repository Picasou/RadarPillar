#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— a1（A1=PillarAttention 底座）
...
CFG="experiments/YAML/a1.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="a1"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_a1"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:a1] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— f3（F3 底座(5frames)）
...
CFG="experiments/YAML/f3.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="f3"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_f3"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:f3] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

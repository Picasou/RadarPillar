#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— b4（B4=[64,64,64] 降 bs 8）
...
CFG="experiments/YAML/b4.yaml"
BS=8
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="b4"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_b4"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:b4] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— b3（B3=[64,128,256] 降 bs 8）
...
CFG="experiments/YAML/b3.yaml"
BS=8
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="b3"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_b3"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:b3] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

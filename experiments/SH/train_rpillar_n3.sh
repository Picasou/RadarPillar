#!/bin/bash
# train_rpillar_n3.sh — 训练入口 (auto-generated from train.sh.template)
#
# 公共模板: .claude/skills/model-train/templates/train.sh.template
# 想改: 先改模板, 再 regen 全部 train_rpillar_*.sh
#
# 用法 (前台):
#   bash experiments/SH/train_rpillar_n3.sh
# 用法 (后台):
#   nohup bash experiments/SH/train_rpillar_n3.sh > /tmp/n3.log 2>&1 &
...
# ============================================================
: "${MODEL:=rpillar_n3}"
: "${CFG_FILE:=experiments/YAML/n3.yaml}"
: "${EXTRA_TAG:=n3}"
: "${BATCH_SIZE:=8}"   # n3=DCNv3 纯 pytorch 显存偏高, bs=16 爆满; bs=8 正常
: "${WORKERS:=2}"
: "${EPOCHS:=80}"
: "${GPU:=0}"

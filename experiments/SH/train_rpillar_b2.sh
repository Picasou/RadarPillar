#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— b2（B2=[32,64,128]）
...
CFG="experiments/YAML/b2.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="b2"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_b2"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:b2] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

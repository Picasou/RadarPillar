#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— e3（E3 +VDC）
...
CFG="experiments/YAML/e3.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="e3"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_e3"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:e3] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

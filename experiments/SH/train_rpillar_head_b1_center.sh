#!/bin/bash
# train_rpillar_head_b1_center.sh — head 阶段实验 b1 + RadarNeXtCenterHead
# 派生自 train_rpillar_b1.sh；仅改 MODEL / CFG_FILE / EXTRA_TAG / OUTPUT_ROOT

# ============================================================
#  默认值 (env 覆盖优先)
# ============================================================
: "${MODEL:=rpillar_head_b1_center}"
: "${CFG_FILE:=experiments/YAML/head_b1_center.yaml}"
: "${EXTRA_TAG:=head_b1_center}"
: "${BATCH_SIZE:=8}"
: "${WORKERS:=2}"
: "${EPOCHS:=80}"
: "${GPU:=0}"

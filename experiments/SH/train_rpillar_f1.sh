#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— f1（F1 单帧）
...
CFG="experiments/YAML/f1.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="f1"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_f1"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:f1] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

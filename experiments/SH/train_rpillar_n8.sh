#!/bin/bash
# train_rpillar_n8.sh — n8 = PillarAttn + RepDWC C3[64,128,256] + MDFEN + RadarNeXtCenterHead
#
# 与 n7 架构同（paper 容量 + MDFEN），仅 head 不同（CenterHead 才对账 paper baseline）
# 目标: 干净对账 baseline_radarnext_mdfen (stage0: Car 32.56 mAP 45.46)
#
# 用法 (前台):
#   bash experiments/SH/train_rpillar_n8.sh
# 用法 (后台):
#   nohup bash experiments/SH/train_rpillar_n8.sh > /tmp/n8.log 2>&1 &

# ============================================================
#  默认值 (env 覆盖优先)
# ============================================================
: "${MODEL:=rpillar_n8}"
: "${CFG_FILE:=experiments/YAML/n8.yaml}"
: "${EXTRA_TAG:=n8}"
: "${BATCH_SIZE:=8}"   # MDFEN DCNv3 显存大, bs=8
: "${WORKERS:=2}"
: "${EPOCHS:=80}"
: "${GPU:=0}"

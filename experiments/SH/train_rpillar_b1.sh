#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— b1（B1=[32,32,32]）
...
CFG="experiments/YAML/b1.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="b1"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_b1"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:b1] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— e2（E2 底座(9维)）
...
CFG="experiments/YAML/e2.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="e2"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_e2"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:e2] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

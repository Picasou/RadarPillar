#!/bin/bash
# train_rpillar_rp_mask.sh — anchor head + pillar mask 实验
# a0 主线 (AnchorHeadSingle) 派生；head 换成 RadarPillarAnchorHeadSingle，
# 推理期只保留非空 pillar 位置的 anchor 候选，减少 NMS 输入。

# ============================================================
#  默认值 (env 覆盖优先)
# ============================================================
: "${MODEL:=rpillar_rp_mask}"
: "${CFG_FILE:=experiments/YAML/rp_mask_a0.yaml}"
: "${EXTRA_TAG:=rp_mask}"
: "${BATCH_SIZE:=8}"
: "${WORKERS:=2}"
: "${EPOCHS:=80}"

#!/bin/bash
# RPiN 阶段2 补强完整训练 .sh（80 epochs）— b8（RepDWCNoneBackbone，OUT_CHANNELS=[64, 64, 64]，对齐 standard b4）
...
CFG="experiments/YAML/b8.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="b8"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_b8"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:b8] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

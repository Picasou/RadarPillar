#!/bin/bash
# train_rpillar_rp_mask_center.sh — center head + pillar mask 实验
# head_center 派生；head 换成 RadarPillarCenterHead，
# 推理期只保留非空 pillar 位置的 heatmap 峰值，减少 NMS 输入。

# ============================================================
#  默认值 (env 覆盖优先)
# ============================================================
: "${MODEL:=rpillar_rp_mask_center}"
: "${CFG_FILE:=experiments/YAML/rp_mask_center.yaml}"
: "${EXTRA_TAG:=rp_mask_center}"
: "${BATCH_SIZE:=8}"
: "${WORKERS:=2}"
: "${EPOCHS:=80}"
: "${GPU:=0}"

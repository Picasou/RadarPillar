#!/bin/bash
# train_rpillar_n5.sh — 训练入口 (auto-generated from train.sh.template)
#
# 公共模板: .claude/skills/model-train/templates/train.sh.template
# 想改: 先改模板, 再 regen 全部 train_rpillar_*.sh
#
# 用法 (前台):
#   bash experiments/SH/train_rpillar_n5.sh
# 用法 (后台):
#   nohup bash experiments/SH/train_rpillar_n5.sh > /tmp/n5.log 2>&1 &

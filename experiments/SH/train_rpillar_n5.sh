#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— n5（N5=RepDWC+FPN 降 bs 8）
...
CFG="experiments/YAML/n5.yaml"
BS=8
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="n5"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_n5"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:n5] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

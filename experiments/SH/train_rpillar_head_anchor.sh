#!/bin/bash
# train_rpillar_head_anchor.sh — 训练入口 (auto-generated from train.sh.template)
#
# 公共模板: .claude/skills/model-train/templates/train.sh.template
# 想改: 先改模板, 再 regen 全部 train_rpillar_*.sh
#
# 用法 (前台):
#   bash experiments/SH/train_rpillar_head_anchor.sh
# 用法 (后台):
#   nohup bash experiments/SH/train_rpillar_head_anchor.sh > /tmp/head_anchor.log 2>&1 &
...
: "${CFG_FILE:=experiments/YAML/head_anchor.yaml}"
: "${EXTRA_TAG:=head_anchor}"
: "${BATCH_SIZE:=16}"
: "${WORKERS:=2}"
: "${EPOCHS:=80}"
: "${GPU:=0}"

#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— head_anchor（AnchorHeadSingle）
...
CFG="experiments/YAML/head_anchor.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="head_anchor"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_head_anchor"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:head_anchor] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— head_center（RadarNeXtCenterHead 降 bs 8）
...
CFG="experiments/YAML/head_center.yaml"
BS=8
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="head_center"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_head_center"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:head_center] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— n2（N2=standard+FPN(PPFPN)）
...
CFG="experiments/YAML/n2.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="n2"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_n2"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:n2] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

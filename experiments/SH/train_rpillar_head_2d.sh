#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— head_2d（RadarNeXtCenterHead2D 降 bs 8）
...
CFG="experiments/YAML/head_2d.yaml"
BS=8
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="head_2d"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_head_2d"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:head_2d] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

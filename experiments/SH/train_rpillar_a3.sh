#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— a3（A3=SEDWConv）
...
CFG="experiments/YAML/a3.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="a3"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_a3"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:a3] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

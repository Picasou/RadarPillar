#!/bin/bash
# RPiN 阶段2 补强完整训练 .sh（80 epochs）— b5（RepDWCNoneBackbone，OUT_CHANNELS=[32, 32, 32]，对齐 standard b1）
...
CFG="experiments/YAML/b5.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="b5"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_b5"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:b5] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

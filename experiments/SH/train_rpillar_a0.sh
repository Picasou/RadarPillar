#!/bin/bash
# RPiN 前置完整训练 .sh（80 epochs）— a0（A0=无注意力）
...
CFG="experiments/YAML/a0.yaml"
BS=16
WORKERS=2
EPOCHS=80
GPU=0
EXTRA_TAG="a0"
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_radarpillar_a0"
...
mkdir -p "$OUTPUT_ROOT"
echo "[rpin:a0] log=$LOG  bs=$BS  output_root=$OUTPUT_ROOT"

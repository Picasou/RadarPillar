#!/bin/bash
# train_rpillar_n6.sh — 训练入口 (auto-generated from train.sh.template)
#
# 公共模板: .claude/skills/model-train/templates/train.sh.template
# 想改: 先改模板, 再 regen 全部 train_rpillar_*.sh
#
# 用法 (前台):
#   bash experiments/SH/train_rpillar_n6.sh
# 用法 (后台):
#   nohup bash experiments/SH/train_rpillar_n6.sh > /tmp/n6.log 2>&1 &
...
# ============================================================
: "${MODEL:=rpillar_n6}"
: "${CFG_FILE:=experiments/YAML/n6.yaml}"
: "${EXTRA_TAG:=n6}"
: "${BATCH_SIZE:=8}"   # n6=RadarNeXtMDFEN(RepDWC+MDFEN) 最重, bs=16 显存爆满(~48min/ep); bs=8 仅 2:55/ep (对齐 0722_paper bs=4-8)
: "${WORKERS:=2}"
: "${EPOCHS:=80}"
: "${GPU:=0}"

...
# [cfg 覆盖]
# —— 必改 ——
CFG_FILE="experiments/stage2/next_stage_cfg/vod_radarpillar_a0_b1.yaml"
BATCH_SIZE=16
WORKERS=8
EPOCHS=80
GPU=0
...
# [output 覆写: 让 train/test 直接写到 output/train_log/vod/<datetime>_rpillar_a0_b1/]
OUTPUT_ROOT="output/train_log/vod/$(date +%Y%m%d%H%M)_rpillar_a0_b1"
...
# conda env fallback helper — 本机唯一环境是 base（angle 为原工程死引用）
# 探测顺序: ${DESIRED_ENV:-base} -> base

#!/bin/bash
# train_rpillar_head_b1_2d_lossup.sh — head 阶段实验 b1_2d_lossup
# 派生自 train_rpillar_head_b1_2d.sh；仅改 MODEL / CFG_FILE / EXTRA_TAG / OUTPUT_ROOT

# ============================================================
#  默认值 (env 覆盖优先)
# ============================================================
: "${MODEL:=rpillar_head_b1_2d_lossup}"
: "${CFG_FILE:=experiments/YAML/head_b1_2d_lossup.yaml}"
: "${EXTRA_TAG:=head_b1_2d_lossup}"
: "${BATCH_SIZE:=8}"
: "${WORKERS:=2}"
: "${EPOCHS:=80}"
: "${GPU:=0}"

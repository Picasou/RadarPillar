#!/bin/bash
# train_rpillar_head_b1_abl_h.sh — 消融配置 Y：b1 + RadarNeXtCenterHeadAblH（2d + h 监督）
# 派生自 train_rpillar_head_b1_2d.sh；仅改 MODEL / CFG_FILE / EXTRA_TAG / OUTPUT_ROOT
# 目的：在 2d(48.14) 基础上加回 h 监督（dim 3 通道），隔离 h 对 BEV 的贡献。对照 center(51.86)/AblZ。

# ============================================================
#  默认值 (env 覆盖优先)
# ============================================================
: "${MODEL:=rpillar_head_b1_abl_h}"
: "${CFG_FILE:=experiments/YAML/head_b1_abl_h.yaml}"
: "${EXTRA_TAG:=head_b1_abl_h}"
: "${BATCH_SIZE:=8}"
: "${WORKERS:=2}"
: "${EPOCHS:=80}"
: "${GPU:=0}"

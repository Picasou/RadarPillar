#!/bin/bash
# train_rpillar_e1.sh — 训练入口 (auto-generated from train.sh.template)
#
# 公共模板: .claude/skills/model-train/templates/train.sh.template
# 想改: 先改模板, 再 regen 全部 train_rpillar_*.sh
#
# 用法 (前台):
#   bash experiments/SH/train_rpillar_e1.sh
# 用法 (后台):
#   nohup bash experiments/SH/train_rpillar_e1.sh > /tmp/e1.log 2>&1 &
...
: "${CFG_FILE:=experiments/YAML/e1.yaml}"
: "${EXTRA_TAG:=e1}"
: "${BATCH_SIZE:=16}"
: "${WORKERS:=2}"
: "${EPOCHS:=80}"

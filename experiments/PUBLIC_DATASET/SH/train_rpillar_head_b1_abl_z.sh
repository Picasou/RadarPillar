#!/bin/bash
# train_rpillar_head_b1_abl_z.sh — 消融配置 X：b1 + RadarNeXtCenterHeadAblZ（2d + z 监督）
# 派生自 train_rpillar_head_b1_2d.sh；仅改 MODEL / CFG_FILE / EXTRA_TAG / OUTPUT_ROOT
# 目的：在 2d(48.14) 基础上加回 z 监督，隔离 z 对 BEV 的贡献。对照 center(51.86)/AblH。

# ============================================================
#  默认值 (env 覆盖优先)
# ============================================================
: "${MODEL:=rpillar_head_b1_abl_z}"
: "${CFG_FILE:=experiments/YAML/head_b1_abl_z.yaml}"
: "${EXTRA_TAG:=head_b1_abl_z}"
: "${BATCH_SIZE:=8}"
: "${WORKERS:=2}"
: "${EPOCHS:=80}"
: "${GPU:=0}"

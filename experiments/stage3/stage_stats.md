...
|---|---|---|---|---|---|---|---|---|---|---|---|
| **n1** ⭐ | standard BaseBEV [32,32,32] | **无**（直连 head） | 8 | 72 | 38.76 | 34.47 | 62.11 | 45.11 | N/A | 186,600 | 35.274 G |
...
| n6 | RepDWC [64,64,64] DWConv | MDFEN CH=[64,64,64,128,64,128,256] FUSE=[128,128,128] | 8 | 79 | 36.96 | 31.08 | 60.34 | 42.79 | N/A | 792,600 | 89.875 G |
...
| **n7** | RepDWC [64,128,256] DWConv | MDFEN CH=[64,128,256,128,64,128,256] FUSE=[128,128,128] | 8 | 78 | 34.72 | 30.27 | 63.05 | 42.68 | N/A | 1,175,700 | 105.384 G |
...
## 8-04~05 重训更新（n1 / n6 / n7）

- **触发**：n1 由 bs=16 统一到 bs=8（与 n3/n6/n7 同协议）；n6/n7 同 bs=8 下 seed 由 False 改为 True（FIX_RANDOM_SEED）。
- **新 run**：
  - n1: bs=8, ep72, Car 38.76 / Ped 34.47 / Cyc 62.11 / mAP **45.11** (was 48.38 bs=16)
  - n6: bs=8, ep79, Car 36.96 / Ped 31.08 / Cyc 60.34 / mAP **42.79** (was 43.09 seed=False)
  - n7: bs=8, ep78, Car 34.72 / Ped 30.27 / Cyc 63.05 / mAP **42.68** (was 43.35 seed=False)
- **关键 caveat**：
  1. **n1 mAP 48.38 → 45.11（−3.27）** 主要由 Cyclist −6.57（68.68→62.11）拉动；Ped/Car 也微降。**待核对 val pkl 聚合口径是否与旧 run 一致**（见 memory vod-val-pkl-aggregation）。若口径有变，此 mAP 跌幅可能为假性。
  2. **新 n1 (45.11) < 旧 n4 (45.66) by mAP**——但 n4 是 bs=16 旧 run，**未重训**。在 bs=8 协议下，n4 缺席公平对照。**待决：是否要重训 n2/n4/n5 一起到 bs=8 seed=True 才能给 D13 干净结论？** 当前 D13 裁决（n1 ⭐）仍基于历史协议，下文「关键发现」数字未改。
  3. **D13 裁决保留原貌**：本节末「关键发现」「D13 裁决 → neck*=无 neck（n1）」仍是原协议下的结论，**不视为失效**。新 run 仅替换表中数字与 model_store 路径；裁决复盘需等 n2/n4/n5 同协议补跑后另开 issue。
- **历史路径未删**：旧 resbag 仍在 `output/train_log/vod/202608010053_rpillar_n1_n1/`、`202608010053_rpillar_n6_n6/`、`202608022326_rpillar_n7_n7/`。

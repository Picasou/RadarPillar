#!/bin/bash
# ============================================================
# 本格点为 §8.3「确定性复用 stage2 胜者」：n1=b1（standard C1[32,32,32]）
# 不重训；stage3 stage_stats 直接复用 stage2 产物（ckpt/eval）。
# 此脚本仅作模板保留——执行它将重训一个等价模型。
# ============================================================
# train_rpillar_n1.sh — 训练入口 (auto-generated from train.sh.template)
#
# 公共模板: .claude/skills/model-train/templates/train.sh.template
# 想改: 先改模板, 再 regen 全部 train_rpillar_*.sh
#
# 用法 (前台):
#   bash experiments/SH/train_rpillar_n1.sh
# 用法 (后台):
#   nohup bash experiments/SH/train_rpillar_n1.sh > /tmp/n1.log 2>&1 &
...
# ============================================================
: "${MODEL:=rpillar_n1}"
: "${CFG_FILE:=experiments/YAML/n1.yaml}"
: "${EXTRA_TAG:=n1}"
: "${BATCH_SIZE:=16}"
: "${WORKERS:=2}"
: "${EPOCHS:=80}"

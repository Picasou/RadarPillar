...
# eg. bash experiments/MC_DATASET/SH/train_msr_pp2s32_ch2d_trunk.sh
# MSR (MC_Single_Radar) RadarPillar 行37 BaseBEVBackbone 两级(3,5)等通道(32,32) + ch2d trunk head 变体
...
# —— 必改 ——
CFG_FILE="experiments/MC_DATASET/YAML/msr_pp2s32_ch2d_trunk.yaml"
BATCH_SIZE=8          # MSR 点云密集 + 8G 显存,保守起步;OOM 则降到 2
WORKERS=4
EPOCHS=80
GPU=0
EXTRA_TAG="msr_pp2s32_ch2d_trunk"

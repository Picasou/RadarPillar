#!/usr/bin/env bash
# 速度量测链修复验证: 备份基线 0200/0201 → 重放 full pipeline → GT 对拍 (基线 vs 修复) 指标.
# 指标: 主动态 GT 目标的 coverage / IDSW / 断链段数 / 滞后 median / 速度比 median.
# 用法: bash tools/scripts/test/verify_tracker_velfix.sh [仅跑指标对比 SKIP_REPLAY=1]
set -euo pipefail
cd "$(dirname "$0")/../../.."   # 工程根
...
for cfg_path in sys.argv[2:]:
    for seq in yaml.safe_load(open(cfg_path, encoding='utf-8'))['DATA']['paths']:
        ...
        print('  修复: coverage={cov:.2f} IDSW={idsw} 段数={segs} 滞后med={lag:+.2f}m 速度比={ratio:.2f}'.format(**m1))
        for d in (m0, m1):
            pass
        def acc(dst, m):
            ...
            for k in ('lag', 'ratio'):
                dst[k] = dst.get(k, []) + ([] if m[k] != m[k] else [m[k]])
        globals()['sum_base'] = sum_base or dict(cov=0, idsw=0, segs=0, lag=[], ratio=[])
        globals()['sum_new'] = sum_new or dict(cov=0, idsw=0, segs=0, lag=[], ratio=[])
        acc(globals()['sum_base'], m0)
        acc(globals()['sum_new'], m1)
        globals()['n_seq'] += 1
if n_seq:
    print('=== 汇总 ({} 序列) ==='.format(n_seq))
    for tag, s in (('基线', sum_base), ('修复', sum_new)):

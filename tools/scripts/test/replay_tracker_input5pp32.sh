#!/usr/bin/env bash
# tracker 全链路回灌: msr_input5_pp32_ch2d_trunk (输入 5 特征去 rcs) 模型 + msr_input5_pp32_ch2d_trunk 数据,
# 航迹结果 bin (0200/0201) 落袋回各序列 radar.default/ (overlap=1 覆盖原始对象级输出)。
# 用法: bash tools/scripts/test/replay_tracker_input5pp32.sh
set -euo pipefail
cd "$(dirname "$0")/../../.."   # 工程根

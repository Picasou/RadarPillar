...
#  必改
# ════════════════════════════════════════════════════════════════
CFG_FILE="tools/cfgs/model/vod_models/radarnext/vod_radarnext_fpn.yaml"
BATCH_SIZE=4
WORKERS=2
GPU=0
...
#  用 :="${VAR:=default}" 让 autofinish 可经 env 覆盖 (与 OUTPUT_ROOT 同).
#  默认 False: GPU eval 快得多, 末 20 epoch val 在 CPU 上要数小时, 不现实.
# ════════════════════════════════════════════════════════════════
: "${CPU_EVAL:=False}"
...
#  用 :="${VAR:=default}" 让 autofinish 可注入 EVAL_MODE=all (末 20 epoch val).
: "${EVAL_MODE:=single}"

...
数据集相关的配置位于 [tools/cfgs/dataset_configs](../tools/cfgs/dataset_configs)，
...
* 注意：如果你已有 `pcdet v0.1` 生成的数据 infos，可以选择复用旧 infos 并在 `tools/cfgs/dataset_configs/kitti_dataset.yaml` 中将 `DATABASE_WITH_FAKELIDAR` 选项设为 `True`。另一种选择是重新生成 infos 和 gt database，并保持配置不变。
